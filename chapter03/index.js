...
//===============================//
  var spaceCubeDetector = function (length, width, height) {
    if (length==width && length==height) {
      //you can't do conditionals with more than two:
      //length==width==height does not work
...
Exer.scopeExample = function() {
  ...
  var skip = function() {
    var foo = 5;
    console.log(foo); // Scope 1: What value of foo is logged? 5
    quux = foo *  bar; // 5*7
    console.log(quux); // Scope 2: What value of quux is logged? 35
    return bar;
  };

  var scoop = function(quux) {
    console.log(quux); // Scope 4: What value of quux is logged? I see that quux is
    //redefined when scoop changes it's value to foo (foo equal to the return of skip).
    //still not comfortable understanding the command flow. Seems the computer would have
    //logged quux here as 35 before continuing to the below commands.
  };

  var scrap = function() {
    console.log(quux); // Scope 5: What value of quux is logged? 35
  };

  foo = skip(); //
  scoop(foo); // Scope 3: What value of foo is passed to the function Scoop? 7
  scrap();
...
Exer.nestExample = function() {
  ...
  var nest = function(foo) { // local foo=5318008)

    var bar = 11; //local

    var finch = function(foo) { // local foo=17
      console.log(foo); // Nest 8: What value of foo is logged?
      bar = foo; // nest local bar now 17
      quux = foo;// global quux now 17
    };
    ...
    console.log(quux); // Nest 12: What value of quux is logged? //robin
    //called so exact same as quux from robin, which is 13
...
Exer.recursion1 = function(code, controller) {
   var ai = function(code, controller) {
      controller(code.curr());
      if (code.next()==code.curr()) {
        ai(code, controller);
      }
    }
    ai(code, controller);
...
Exer.growing1 = function (pigPenSide, chickenPenSide, sheepPenSide) {
  var squared = function (animal, size) {
  console.log("The " + animal + " need " + size * size + "sq. ft.")
  }
squared("pigs", pigPenSide);
squared("chickens", chickenPenSide);
squared("sheep", sheepPenSide);
};
